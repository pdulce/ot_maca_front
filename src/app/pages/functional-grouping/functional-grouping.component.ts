 ...
 */
@Component({
	selector: 'app-functional-grouping',
	templateUrl: './functional-grouping.component.html'
})
export class FunctionalGroupingComponent implements OnInit, AfterViewInit {
	...
	filter(type: any, ev: any) {
		if (type === 'toggle') {
			...
			setTimeout(() => {
				this.catalogService.getAllFunctionalGroupingItems(params)
					.subscribe(
						(res: any) => {
	 ...
	 */
	addNew() {
		const dialogRef = this.dialog.open(FunctionalGroupingAddnewDialog, {
			data: {
				name: 'new',
	...
	itemActions(type: any, item: any) {
		if (type === 'edit') {
			const dialogRef = this.dialog.open(FunctionalGroupingEditDialog, {
				data: {
					name: 'edit',
					sessionData: this.sessionData,
					item: item,
 ...
 */
@Component({
	selector: 'functional-grouping.addnew',
	templateUrl: 'functional-grouping.addnew.html',
})
export class FunctionalGroupingAddnewDialog implements OnInit, OnDestroy, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
	...
	constructor(
		public dialog: MatDialog,
		public dialogRef: MatDialogRef<FunctionalGroupingAddnewDialog>,
	 ...
	 */
	selects() {
		this.catalogService.getFreeAplicaciones()
			.subscribe((res: any) => {
	 ...
	 */
	addNewAditional() {
		const dialogRef = this.dialog.open(FunctionalGroupingAddnewAditionalDialog, {
			data: {
				name: 'new',
				sessionData: this.sessionData,
				selects: {
 ...
 */
 @Component({
	selector: 'functional-grouping.addnew-aditional',
	templateUrl: 'functional-grouping.addnew-aditional.html',
})
export class FunctionalGroupingAddnewAditionalDialog implements OnInit, OnDestroy, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
 ...
 */
@Component({
	selector: 'functional-grouping.edit',
	templateUrl: 'functional-grouping.edit.html'
})
export class FunctionalGroupingEditDialog implements OnInit, OnDestroy, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
			...
			if (this.actionForm.valid) {
				...
				if (form.cappCode.length !== 6) {
					...
					if (this.aditionalData.length) {
						...
						this._snackBar.open('Debe seleccionar al menos una aplicación ', 'Cerrar', {
							horizontalPosition: 'center',
							verticalPosition: 'top',
							duration: 3000
						});
