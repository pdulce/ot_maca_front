 ...
 */
@Component({
	selector: 'app-promotable-item',
	templateUrl: './promotable-item.component.html'
})
export class PromotableItemComponent implements OnInit, AfterViewInit {
	...
	filter(type: any, ev: any) {
		if (type === 'toggle') {
			...
			setTimeout(() => {
				this.catalogService.getAllPromotableItems(params)
					.subscribe(
						(res: any) => {
	 ...
	 */
	addNew() {
		const dialogRef = this.dialog.open(PromotableItemAddnewDialog, {
			data: {
				name: 'new',
	...
	itemActions(type: any, item: any) {
		if (type === 'edit') {
			const dialogRef = this.dialog.open(PromotableItemEditDialog, {
				data: {
					name: 'edit',
					sessionData: this.sessionData,
					item: item,
 ...
 */
@Component({
	selector: 'promotable-item.addnew',
	templateUrl: 'promotable-item.addnew.html',
})
export class PromotableItemAddnewDialog implements OnInit, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
	...
	constructor(
		public dialog: MatDialog,
		public dialogRef: MatDialogRef<PromotableItemAddnewDialog>,
	 ...
	 */
	selects() {
		this.catalogService.getAplicaciones()
			.subscribe((res: any) => {
	 ...
	 */
	addNewAditional() {		
		const dialogRef = this.dialog.open(PromotableItemAddnewAditionalDialog, {
			data: {
				name: 'edit',
				sessionData: this.sessionData,
				selects: {
		...
		dialogRef.afterClosed().subscribe(result => {
			//console.log("addNewAditional:app elegida: " + JSON.stringify(result.res[0].id));
			let idApp: number;
			idApp = result.res[0].id;
			this.catalogService.getElementById(idApp)
			.subscribe((res: any) => {
				let application = res;
				this.appCappCode = application.cappCode;
				//console.log("CAPP Code de la aplicacion seleccionada es: " + this.appCappCode);
				//console.log("addNewAditional:application recibida tiene estos attrs: " + JSON.stringify(application.attributeValuesCollection));
				let axisAtributes = this.data.selects ? this.data.selects.axisAtributes : [];
				axisAtributes.map((x: any) => {					
					let temp = application.attributeValuesCollection.find((element: any)=> element.axisAttributeId === x.id);					
					//console.log('element.axisAttributeId: ' + x.id);
					if (temp){
						x.value = temp.userValue;
						//console.log('element.axisAttributeId('+ x.id +'):x.value ' + x.value);
						...
						let inputSelectList = [];
						if (temp.domainValues && temp.domainValues.length > 0) { 
						...
						//console.log('element.axisAttributeId('+ x.id +'):x.valuesOfSelect ' + x.valuesOfSelect);
					}
	...
	submit(type: any) {
		if (type === 'save') {
			if (this.actionForm.valid) {
				...
				if (form.cappCode.length !== 8) {
					this._snackBar.open('El Código CAPP tiene que tener exactamente 8 caracteres alfanuméricos', 'Cerrar', {
						horizontalPosition: 'center',
						verticalPosition: 'top',
						duration: 3000
					});
					...
					for (let item of table) {
						...
						const a: any = {							
							...
							domainValueId: item.valuesOfSelect ? item.valuesOfSelect[0] : null,																
				...
				let applicationList = this.aditionalData.map((i: any) => { return {id: i.id} });
				let identificadorParent : number;
				identificadorParent = applicationList.length ? applicationList[0].id : null;
				if (applicationList.length > 1 || applicationList.length < 1) {
					this._snackBar.open('Debe seleccionar una sola aplicación para este elemento promocionable', 'Cerrar', {
						horizontalPosition: 'center',
						verticalPosition: 'top',
						duration: 3000
					});
				...
				if (keepGoing) {
					...
					let params = form;
					params.catalogElementTypeId = 1;
					params.attributeValuesCollection = array;
					params.parentElement = applicationList.length ? applicationList[0] : null;
 ...
 */
@Component({
	selector: 'promotable-item.addnew-aditional',
	templateUrl: 'promotable-item.addnew-aditional.html',
})
export class PromotableItemAddnewAditionalDialog implements OnInit, OnDestroy, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
	...
	constructor(
		...
		//console.log("aditionalData.length: " + aditionalData.length);
	...
	submit(type: any) {
		if (type === 'save') {
			if (this.selection.selected.length === 0) {
				...
				this._snackBar.open('Debe seleccionar una aplicación para este elemento promocionable', 'Cerrar', {
					horizontalPosition: 'center',
					verticalPosition: 'top',
					duration: 3000
				});
 ...
 */
@Component({
	selector: 'promotable-item.edit',
	templateUrl: 'promotable-item.edit.html'
})
export class PromotableItemEditDialog implements OnInit, AfterViewInit {
	@ViewChild(MatPaginator) paginator!: MatPaginator;
	@ViewChild(MatSort) sort!: MatSort;
	...
	constructor(
		public dialog: MatDialog,
		public dialogRef: MatDialogRef<PromotableItemEditDialog>,
		@Inject(MAT_DIALOG_DATA) public data: any,
		private _fb: FormBuilder,
		private _snackBar: MatSnackBar,
		private catalogService: CatalogService,
		...
		// Aditional
		this.aditionalData = this.data.item.parentElement ? [this.data.item.parentElement] : [];
		this.dataSourceAditional.data = this.aditionalData;
	...
	submit(type: any) {
		if (type === 'save') {
			if (this.actionForm.valid) {
				...
				identificadorParent = applicationList.length ? applicationList[0].id : null;
				if (applicationList.length > 1 || applicationList.length < 1) {
					this._snackBar.open('Debe seleccionar una y sola aplicación para este elemento promocionable', 'Cerrar', {
						horizontalPosition: 'center',
						verticalPosition: 'top',
						duration: 3000
					});
				...
				if (keepGoing) {
					...
					this.catalogService.updateItem(params)
						.subscribe(
							...
							error => {
								this.submitLoading = false;
								//console.log("Error actualizando eleme.promo: " + JSON.stringify(error));
								//console.log("Error msg: " + error.error.errors[0].message);
								this._snackBar.open(error.error.errors[0].message ? error.error.errors[0].message : 'Error de aplicación', 'Cerrar', {
									horizontalPosition: 'center',
									verticalPosition: 'top',
									duration: 3000
